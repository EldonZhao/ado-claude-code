...
Enables work item sync, AI-assisted planning, instructions/TSG management, and troubleshooting via CLI commands.

## Quick Start
```bash
npm install
npm run build
node dist/cli.js setup init --url=https://myorg.visualstudio.com/MyProject
# or: node dist/cli.js setup init --organization=https://dev.azure.com/myorg --project=MyProject
...
- `workitems` — get, list, create, update, query, plan (code), workitem-plan (breakdown), summary
- `sync` — pull, push, full
- `clear` — [--confirm]
- `instructions` — create, get, update, list, search, execute, score, diagnose, analyze, suggest, run

## Architecture
- `src/cli.ts` — CLI entry point (arg parsing + routing)
- `src/cli/` — CLI handler modules
  - `workitems.ts` — Work item CRUD, query, plan (code), workitem-plan (breakdown), summary
  - `sync.ts` — Bidirectional sync
  - `clear.ts` — Clear synced work items
  - `instructions.ts` — TSG CRUD, search, execute, score, diagnose, analyze, suggest, run
...
- `tests/cli/` — CLI handler tests (clear, sync, workitems plan, workitems workitem-plan, workitems summary)
