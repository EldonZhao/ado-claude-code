...
async function handleUpdate(args: string[]): Promise<void> {
  ...
  if (isComplete) {
    ...
    try {
      const commentText = `Work item marked as completed by Claude at ${new Date().toISOString()}`;
...
async function handlePlan(args: string[]): Promise<void> {
  ...
  if (!skipUpdate) {
    ...
    try {
      const commentText = `Code plan generated by Claude at ${new Date().toISOString()}\n\n${guidance}`;
      commentAdded = await client.addComment(id, commentText);
    } catch (err) {
...
async function handleWorkitemPlan(args: string[]): Promise<void> {
  ...
  if (!skipUpdate) {
    ...
    // Add comment
    try {
      const commentText = `Work item breakdown plan generated by Claude at ${new Date().toISOString()}`;
      commentAdded = await client.addComment(id, commentText);
    } catch (err) {
      process.stderr.write(`Warning: Failed to add comment: ${err}\n`);
    }
  ...
  if (!skipUpdate && created.length > 0) {
    try {
      const listing = created.map((c) => `#${c.id} ${c.title}`).join(", ");
      const commentText = `Created ${created.length} child item(s): ${listing}`;
